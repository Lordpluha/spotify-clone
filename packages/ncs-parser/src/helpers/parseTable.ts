...
export default (table: Element): Song[] => {
  ...
  const songs = Array.from(rows).map<Song>((row) => {
    const [playerCol, genreCol, _imgCol, mainCol, tagsCol, _dateCol, tracksCol, _downloadCol] =
    ...
    const name = player?.getAttribute('data-track') || 'Untitled'
    ...
    const artists: Artist[] = artistsEl
      ? artistsEl.split(', ').map((artist) => {
          ...
          const name = artistEl?.innerHTML || 'Undefined Artist'
          return { name, url }
        })
      : []
