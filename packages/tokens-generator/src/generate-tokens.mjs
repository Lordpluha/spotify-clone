  ...
  return {
      ...
      const parts = [
        ...config.palette.scales.map(({ name, key }) =>
          cssBuilder.section(
            name,
            tokens.palette[key],
            (shade, value) => `  --color-${key}-${shade}: ${value};`,
          ),
        ),
        cssBuilder.section(
          'Pure Colors',
    ...
    typography: () => {
      ...
      const mainSections = config.typography.sections.map((section) =>
        cssBuilder.section(
          section.name,
          tokens.typography?.[section.tokenKey],
          createFormatter(section),
        ),
      )

      const semanticParts =
        config.typography.semantic.enabled && tokens.typography?.semantic
          ? [
              '  /* Semantic Typography Tokens */',
              '',
              ...Object.entries(tokens.typography.semantic).flatMap(([element, props]) => {
                const elementName =
                  config.typography.semantic.transformKey === 'kebab'
                    ? toKebabCase(element)
                    : element
                return [
                  `  /* ${capitalize(element)} */`,
                  ...Object.entries(props).map(([prop, value]) => {
                    const propName =
                      config.typography.semantic.transformKey === 'kebab' ? toKebabCase(prop) : prop
...
export async function generateTokens(config) {
  ...
  Object.entries(config.files).forEach(([filename, fileConfig]) => {
    ...
    if (fileConfig.generator === 'themes') {
      ...
      const parts = [cssBuilder.header(title, description), ...generatorFn(), cssBuilder.close()]
