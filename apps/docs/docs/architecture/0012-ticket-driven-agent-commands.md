...
**Partially superseded** by [ADR-0016](./0016-live-github-queries.md): `/br-sync-ticket`
and `/br-sync-board`, described below as mirroring GitHub state into `obsidian/Tickets/`
...
the time; the current command set is three (`/br-take-ticket`, `/br-implement`,
`/br-sync-docs`). Everything else in this ADR (the specialist-agent split, model
assignments, `/br-implement` routing, self-contained-command pattern, confirm-before-mutate
...
"self-contained, no agent file" decision for `/br-take-ticket` and `/br-sync-docs` (in
"Alternatives considered" below) is reversed — both now dispatch to a new read-only
discovery agent (`br-ticket`, `br-docs`) by default. `/br-implement`'s `--agent`-gated
...
ADR-0008 defined five coarse dev-task commands (`/br-plan`, `/br-develop`, `/br-debug`,
`/br-review`, `/br-test`), each a 1:1 pairing of a slash command with its own heavy `--agent`
...
- Replace the five `br-*` **commands** with five ticket-driven commands:
  `/br-take-ticket`, `/br-implement`, `/br-sync-ticket`, `/br-sync-board`, `/br-sync-docs`.
  Commands are the entrypoints — they decide scope and stay in the current session by
  default.
- Keep the classic specialist **agents**, renamed for clarity: `br-planner` (was `br-plan`),
  `br-developer` (was `br-develop`), `br-debugger` (was `br-debug`), `br-tester` (was
  `br-test`), `br-reviewer` (unchanged name). Their bodies are otherwise unchanged from
  ADR-0008: `br-planner` is plan-only and asks clarifying questions; `br-developer` writes
  code and auto-invokes `br-reviewer` on diffs over 100 lines/5 files; `br-debugger`
  reproduces → isolates → fixes → verifies; `br-tester` writes/runs one focused spec;
  `br-reviewer` does the mechanical pass + architecture checklist walk.
- Model is fixed per agent, in its own frontmatter: `br-planner` → Fable (light,
  fast-turnaround planning); `br-developer` → Sonnet (routine implementation); `br-debugger`,
  `br-tester`, `br-reviewer` → Opus (the three verification-heavy roles, where a missed edge
...
- What changed is the **pairing**: these five no longer each have their own 1:1 slash
  command. `/br-implement [--agent] [--plan] [--review]` is the single entrypoint that
  routes to whichever specialist the task needs (`br-planner` for `--plan` or a non-trivial
  task, `br-developer` for a feature, `br-debugger` for a bug, `br-reviewer` for `--review`
  or a large diff), or does the equivalent narrow-scope work itself in-session when no
  isolated agent is warranted.
- `/br-take-ticket` finds a GitHub issue (via `gh`, from the GitHub Projects board or a
  direct reference), moves its board card, and checks out a branch.
- `/br-sync-ticket` and `/br-sync-board` mirror GitHub issue/PR/board state into
  `obsidian/Tickets/` and `obsidian/Board/board.md`. Both are strictly read-only on GitHub.
  `/br-sync-docs` finds and (with confirmation) fixes drift between `apps/docs/` and the
  rule/ADR sources — see [ADR-0013](./0013-docs-sync.md).
- `/br-take-ticket`, `/br-sync-ticket`, `/br-sync-board`, and `/br-sync-docs` are **fully
  ...
  connected). Only `/br-implement` has named specialist agents behind it.
- Every command and every agent (the five `/br-implement` specialists plus the four
...
- `/br-take-ticket` and `/br-implement` (whether in-session or via a dispatched specialist)
  ...
  stays at the `/br-implement` orchestration level.
...
- The specialist personas (plan/develop/debug/test/review) keep their full depth —
  `br-reviewer`'s architecture-checklist walk and `br-debugger`'s reproduce-first discipline
  ...
  lives in `/br-implement` instead of being implicit in "which command did you type."
  `/br-implement`'s in-session (no `--agent`) path still does lint/type-check as the
...
- Changing any of the five agents or five commands requires synchronising: the command doc
  (and its agent doc, for `/br-implement`'s five), `AGENTS.md`'s command table,
...
- **Fold planning/debug/review/test into `br-implement` as inline, non-specialist work** —
...
- **Keep `br-*` (one command per agent) alongside the four new ticket commands** —
  ...
  of `/br-implement`'s routing gets the same specialist depth without the fragmentation.
- **Let `/br-implement` push and open PRs without confirmation** — rejected; opening/updating
...
- **Give `br-take-ticket`/`br-sync-ticket`/`br-sync-board`/`br-sync-docs` their own agent
