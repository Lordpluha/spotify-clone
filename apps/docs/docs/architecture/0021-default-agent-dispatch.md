...
- `/sp-implement` defaulted to in-session work, dispatching to `sp-planner`/`sp-developer`/
  `sp-debugger`/`sp-tester`/`sp-reviewer` only when the user passed `--agent` (or, for
  `sp-planner`, when the task looked non-trivial — a condition that in practice created a
...
- `/sp-take-ticket` and `/sp-sync-docs` were deliberately left without their own agent
...
- **`/sp-implement`** now dispatches to the matching specialist by default. The `--agent`
  flag is removed (dispatch no longer needs gating); a new `--session` flag is the explicit
  opt-out for narrow in-session work. The `sp-planner` ambiguity ADR-0012 left open is
  resolved the same direction: non-trivial tasks always get a plan first, no flag needed.
- **`/sp-take-ticket`** and **`/sp-sync-docs`** each get a new, narrower read-only discovery
  agent — `sp-ticket` and `sp-docs` respectively (`.claude/agents/sp-ticket.md`,
  `.claude/agents/sp-docs.md`) — reversing ADR-0012's "give them their own agent files"
...
- **Ordinary tasks outside any `/sp-*` command** — a task in normal conversation that touches
  `apps/api/` or `apps/web-player/` code — also route to the matching `sp-*` specialist via
  the Agent tool by default now, not just work handled through `/sp-implement`. Work
...
- `sp-ticket` and `sp-docs` duplicate some prose with their respective commands (the same
...
- **Only change `/sp-implement`, leave `/sp-take-ticket`/`/sp-sync-docs` and
...
- **Fold `sp-ticket`/`sp-docs`'s logic directly into `sp-implement`'s existing specialists**
