...
export function proxy(request: NextRequest) {
  ...
  if (!token) {
    ...
    if (!isPublicRoute) {
      console.log("Redirecting to login - no token, trying to access protected route:", pathname)
  ...
  if (token) {
    ...
    if (isAuthRoute) {
      console.log("Redirecting to main - user authenticated, on auth route:", pathname)
