...
export const NextInQueue: React.FC = () => {
  ...
  const nextTrack = currentIndex >= 0 && currentIndex < playlist.length - 1 
  ...
  return (
    <div className='bg-surface rounded-lg p-0 overflow-hidden mt-4'>
      <div className='flex items-center justify-between px-4 pt-3 pb-1'>
        <Typography as='p' size='body' className='text-text text-sm font-semibold'>
          Next in queue
        </Typography>
        <Button variant='link' className='text-grey-500 text-xs font-medium hover:underline p-0'>
          Open queue
        </Button>
      </div>
      <div
      ...
      >
        ...
        <div className='flex-1 min-w-0'>
          <Typography as='p' size='body' className='text-text text-sm truncate group-hover:text-green-500 transition-colors'>
            {nextTrack.title}
          </Typography>
          <Typography as='p' size='body' className='text-grey-500 text-xs truncate'>
            {artistName}
          </Typography>
        </div>
      </div>
