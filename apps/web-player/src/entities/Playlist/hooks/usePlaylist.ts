...
export const usePlaylist = (playlistId: string, onSuccess?: (playlist: WithTracks<PlaylistEntity>) => void) =>{
  const query = useQuery(
    ...
    },
    {
      enabled: !!playlistId,
			select: data => {
				const tracks = (data as WithTracks<typeof data>).tracks.map((track) => ({
					...track,
