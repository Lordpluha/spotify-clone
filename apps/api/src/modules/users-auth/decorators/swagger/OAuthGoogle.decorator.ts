...
export function OAuthGoogleCallbackSwagger() {
  return applyDecorators(
    ApiOperation({
      ...
      description:
        'Handled by Google after user consents. On success sets auth cookies and redirects to USER_WEB_HOST (or legacy WEB_HOST). On 2FA required, redirects to /login/2fa with a pending token.',
    }),
    ApiQuery({
      name: 'code',
      required: true,
